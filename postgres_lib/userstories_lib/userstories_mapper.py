...
def build_row_dict(guid,resource_dict):
    ...
    row_dict['UpdatedById'] = 'NULL' if not fnx.is_key_in_dictionary(resource_dict,'updatedBy') else  resource_dict['updatedBy']['id']
    row_dict['CreatedOn'] = '' if not fnx.is_key_in_dictionary(resource_dict,'createdOn') else resource_dict['createdOn']
    row_dict['ResponsibleId'] = 'NULL' if not fnx.is_key_in_dictionary(resource_dict,'owner','id') else resource_dict['owner']['id']
